...
## V11 tactic display correction
- Rebuilt from V9.
- Removed images directly from `tacticCardsReadOnly()` and `tacticBar()`.
- Kept tactic artwork on the pre-case tactic-selection screen.
- Tightened spacing for the text-only in-case tactic cards and sticky tactic bar.
- Verified the rendered in-case tactic markup contains no image elements.
