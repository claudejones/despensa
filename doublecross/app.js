...
function tacticCardsReadOnly(){const s=S();return `<div class="briefTactics">${s.selected.map(id=>`<div class="briefTactic"><div><b>${TACTICS[id].short}</b><small>${TACTICS[id].name}</small><p>${TACTICS[id].desc}</p></div></div>`).join("")}</div>`}
function brief(){
 const c=C(),s=S();
...
function tacticBar(){const s=S();if(!s.selected.length)return"";return `<div class="activeTactics">${s.selected.map(id=>`<button class="tacticChip clickTarget ${s.used[id]?"used":""}" onclick="useTactic('${id}')"><span><b>${TACTICS[id].short}${s.used[id]?" · USED":""}</b><small>${TACTICS[id].name}</small></span></button>`).join("")}</div>`}
