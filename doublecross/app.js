...
const STORE = "doubleCrossSolo_case1v2_v19";
...
function opts(arr){return `<option value="">Choose…</option>${arr.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join("")}`;}
function accuse(){
  return `<div class="sectionHead"><div><div class="kicker">When you are ready</div><h2>Final Theory</h2></div></div>
  ${workspaceSticky()}
  <section class="accuseCard">
    <div class="theorySection coreTheory">
      <div class="theoryHead"><div><span class="theoryStep">SOLVE THE MURDER</span><h3>What really happened?</h3></div><small>3 deductions</small></div>
...
function confirmTheory(){
  ...
  openModal(`<div class="modalHeader"><div><div class="kicker">FINAL THEORY</div><h2>Submit this theory?</h2><p>You will learn how many of your three deductions are correct, but not which ones.</p></div><button class="close clickTarget" onclick="closeModal()">Review</button></div>
  <div class="modalBody"><div class="theoryConfirm"><p><b>Who:</b> ${esc(C.suspects.find(x=>x.id===suspect).name)}</p><p><b>What:</b> ${esc(C.weapons.find(x=>x.id===weapon).name)}</p><p><b>Where:</b> ${esc(C.locations.find(x=>x.id===location).name)}</p></div>
  <div class="modalActions"><button class="secondary clickTarget" onclick="closeModal()">Keep Reviewing</button><button class="primary clickTarget" onclick="submitTheory('${suspect}','${weapon}','${location}')">Submit Theory</button></div></div>`);
}
function submitTheory(suspect,weapon,location){
  ...
  S.result={suspect,weapon,location,core,solved:core===3,attempts:(S.result?.attempts||0)+1};save();closeModal();render();forceTop();
}
function resultHTML(){
  if(!S.result) return "";
  if(!S.result.solved) return `<article class="resultCard unsolved theoryFeedback"><span class="tag">THE THEORY DOESN'T HOLD</span><h3>${S.result.core} of 3 deductions are correct.</h3><p>Something in your theory still conflicts with the evidence. The case will not identify which deduction is wrong.</p><small>Incorrect theories submitted: ${S.result.attempts}</small></article>`;
